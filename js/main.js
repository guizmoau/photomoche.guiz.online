...
MOCHE.events = new Events, MOCHE.noise = new ImprovedNoise, MOCHE.Main = function() {
    ...
    function d() {
        if (MOCHE.Input.getSourceReady()) {
            var e = MOCHE.Input.getSourceSize(),
                o = e.x / e.y,
                t = f[MOCHE.Recorder.getRecordMode()];
    ...
    function m(e) {
        switch (e.keyCode) {
            case 32:
                MOCHE.FX.randomizeFilters()
        }
    }
    return {
        init: function() {
            if (console.log("PHOTO-MOCHE", MOCHE.VERSION), o = -1 < window.location.href.indexOf("?dev"), t = !!("ontouchstart" in window), r = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream, -1 < window.location.href.indexOf("mobile") && (t = !0), function() {
                    var e = !1;
                    try {
            ...
            }), $("#webgl").append(v.domElement), MOCHE.FX.init(), MOCHE.Recorder.init(), MOCHE.Input.init(), window.addEventListener("resize", d, !1), $(document).on("keydown", m, !1), d(), a = $("#intro-logo")
        },
        update: function e() {
            requestAnimationFrame(e), a.css("opacity", Math.random() + .5), MOCHE.Input.getSourceReady() && (MOCHE.events.emit("update"), s.update())
        },
        trace: function(e) {
            $("#debug-text").text(e)
        },
    ...
    function f() {
        (t = new THREE.EffectComposer(MOCHE.Main.getRenderer())).addPass(r), $.each(a, function(e, o) {
            o.on && t.addPass(s[e])
        }), t.addPass(n), n.renderToScreen = !0, m()
    }

    function d() {
        var e = performance.now() / (1e3 * MOCHE.Recorder.getVideoDuration());
        s.wobble.uniforms.time.value = e, s.slices.uniforms.time.value = e, s.rainbow.uniforms.time.value = e, s.badtv.uniforms.time.value = e, s.smear.uniforms.time.value = e, s.scanlines.uniforms.time.value = e, s.barrelBlur.uniforms.time.value = e, s.shake.uniforms.time.value = e, s.glitcher.uniforms.time.value = e, s.noiseDisplace.uniforms.time.value = e, s.rgb.uniforms.angle.value = 6.28 * (MOCHE.noise.noise(e, 99, 0) + .5), s.huesat.uniforms.hue.value = 2 * MOCHE.noise.noise(2 * e, 999, 0), t.render(.1)
    }

    function m() {
        t && t.setSize(MOCHE.Main.renderSize.x, MOCHE.Main.renderSize.y), s.scanlines.uniforms.height.value = MOCHE.Main.renderSize.y, s.linocut.uniforms.resolution.value = MOCHE.Main.renderSize
    }
    return {
        init: function() {
            var e = new THREE.TextureLoader;
            MOCHE.events.on("update", d), MOCHE.events.on("resize", m), r = new THREE.RenderPass(MOCHE.Main.getScene(), MOCHE.Main.getCamera()), n = new THREE.ShaderPass(THREE.CopyShader), (s = {}).badtv = new THREE.ShaderPass(THREE.BadTVShader), s.rgb = new THREE.ShaderPass(THREE.RGBShiftShader), s.pixelate = new THREE.ShaderPass(THREE.PixelateShader), s.slices = new THREE.ShaderPass(THREE.SlicesShader), s.dotmatrix = new THREE.ShaderPass(THREE.DotMatrixShader), s.scanlines = new THREE.ShaderPass(THREE.ScanlinesShader), s.shake = new THREE.ShaderPass(THREE.ShakeShader), s.vignette = new THREE.ShaderPass(THREE.VignetteShader), s.mirror = new THREE.ShaderPass(THREE.MirrorShader), s.dotscreen = new THREE.ShaderPass(THREE.DotScreenShader), s.lut = new THREE.ShaderPass(THREE.LUTShader), s.glow = new THREE.ShaderPass(THREE.GlowShader), s.posterize = new THREE.ShaderPass(THREE.PosterizeShader), s.huesat = new THREE.ShaderPass(THREE.HueSaturationShader), s.brightness = new THREE.ShaderPass(THREE.BrightnessContrastShader), s.polar = new THREE.ShaderPass(THREE.PolarPixelateShader), s.edges = new THREE.ShaderPass(THREE.EdgeShader), s.tilt = new THREE.ShaderPass(THREE.VerticalTiltShiftShader), s.wobble = new THREE.ShaderPass(THREE.WobbleShader), s.smear = new THREE.ShaderPass(THREE.SmearShader), s.barrelBlur = new THREE.ShaderPass(THREE.BarrelBlurShader), s.glitcher = new THREE.ShaderPass(THREE.GlitcherShader), s.noiseDisplace = new THREE.ShaderPass(THREE.NoiseDisplaceShader), s.duoTone = new THREE.ShaderPass(THREE.DuoToneShader), s.rainbow = new THREE.ShaderPass(THREE.RainbowShader), s.solarize = new THREE.ShaderPass(THREE.SolarizeShader), s.linocut = new THREE.ShaderPass(THREE.LinocutShader);
            for (var o = 0; o < v.length; o++) {
                var t = e.load("lut/" + v[o]);
...
}(), MOCHE.Input = function() {
    var r, n = !1,
        e = new THREE.Vector2,
        i = document.createElement("video");

    function o() {
        $("#file1").on("change", t, !1), MOCHE.Main.getIsMobile() ? $("#file1").click() : ($("#input-options").css("display", "none"), $("#cam-prompt").css("display", "none"), TweenMax.to($("#file-prompt"), .6, {
            autoAlpha: 1
    ...
    function l() {
        r.minFilter = THREE.LinearFilter, e = new THREE.Vector2(r.image.width, r.image.height), MOCHE.Recorder.setInputDuration(0), c()
    }

    function v() {
        u(), MOCHE.Recorder.setInputDuration(0)
    }

    function u() {
        (r = new THREE.VideoTexture(i)).minFilter = THREE.LinearFilter, e = new THREE.Vector2(i.videoWidth, i.videoHeight), c(), console.log("webcam dims:", i.videoWidth, i.videoHeight), MOCHE.Recorder.setInputDuration(i.duration), i.play()
    }

    function c() {
        $("#webgl").css("display", "block"), $("#intro").css("display", "none"), $("#save-options").css("display", "block"), $("#controls-holder").css("display", "block"), $("#bottom-controls-holder").css("display", "block"),$("#top-logo-holder").css("display", "block"), $("#drop-target-holder").hide(), n = !0, MOCHE.Main.updateMaterial(r)
    }
    ...
    function u() {
        "gif" === n ? (e = new GIF({
            ...
            var e = MOCHE.Main.getRenderer().domElement.captureStream();
            r = [];
            var o = {
            ...
            MOCHE.FX.getComposer().render(.1);
            var e = MOCHE.Main.getRenderer().domElement.toDataURL("image/jpeg");
            MOCHE.Main.getIsIOS() ? function(e, o) {
    ...
    function c() {
        var e = new Date;
        return "MOCHE-" + e.getFullYear() + "-" + (e.getMonth() + 1) + "-" + e.getDate() + "-" + e.getHours() + "-" + e.getMinutes() + "-" + e.getSeconds()
    }

    function f() {
        $("#btn-record").off("click", u), $("#btn-record").css("cursor", "default"), $("#btn-record").toggleClass("recording"), $("#btn-record").text("Enregistrement..."), $("#progress-bar").show(), $("#progress-bar-bkgnd").show(), TweenMax.fromTo($("#progress-bar"), i, {
            scaleX: 0
        }, {
            scaleX: 1,
    ...
    function d() {
        "gif" === n ? (clearInterval(o), e.render(), $("#btn-record").text("En cours de traitement..."), e.on("finished", function(e) {
            ! function(e) {
    ...
    function p() {
        e.addFrame(MOCHE.Main.getRenderer().domElement, {
            copy: !0,
            delay: a
        })
    }
